...
Care Collective PRP Runner - supports organized directory structure with active/completed/backlog.

Typical usage:
    uv run PRPs/scripts/prp_runner.py --prp test --interactive
    uv run PRPs/scripts/prp_runner.py --prp-path PRPs/active/feature.md --interactive
    uv run PRPs/scripts/prp_runner.py --prp test --output-format json

Arguments:
    --prp-path       Path to a PRP markdown file (overrides --prp)
    --prp            Feature key; searches in PRPs/active/, PRPs/backlog/, then PRPs/
    --folder         Specific folder to search: active, backlog, completed (default: active)
...
ROOT = Path(__file__).resolve().parent.parent.parent  # project root (up from scripts/prp_runner.py to root)
...
def find_prp_file(prp_name: str, folder: str = "active") -> Path:
    """Find a PRP file in the organized directory structure."""
    # Define search order based on folder preference
    if folder == "active":
        search_folders = ["active", "backlog", ""]  # Also check root PRPs folder
    elif folder == "backlog":
        search_folders = ["backlog", "active", ""]
    elif folder == "completed":
        search_folders = ["completed", "active", "backlog", ""]
    else:
        search_folders = [folder, "active", "backlog", ""]
    
    for folder_name in search_folders:
        if folder_name:
            prp_path = ROOT / "PRPs" / folder_name / f"{prp_name}.md"
        else:
            prp_path = ROOT / "PRPs" / f"{prp_name}.md"
...
def main() -> None:
    ...
    parser.add_argument(
        "--prp-path", help="Relative path to PRP file eg: PRPs/active/feature.md"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="active",
        help="Specific folder to search for PRP (default: active)"
    )
    parser.add_argument(
    ...
    if args.prp_path:
        ...
        if not prp_path.is_absolute():
            prp_path = ROOT / args.prp_path
    else:
        prp_path = find_prp_file(args.prp, args.folder)
        if not prp_path:
            sys.exit(f"PRP '{args.prp}' not found in any folder (searched: active, backlog, completed)")

    if not prp_path.exists():
        sys.exit(f"PRP not found: {prp_path}")

    print(f"Running PRP: {prp_path.relative_to(ROOT)}", file=sys.stderr)
