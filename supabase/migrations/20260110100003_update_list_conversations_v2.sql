-- Migration: Update list_conversations_v2 to handle system conversations
-- Adds support for conversations with null help_request_id (system messages)
...
BEGIN
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      -- Flag for system conversations
      'is_system_conversation', (c.help_request_id IS NULL),
      ...
      'other_participant', jsonb_build_object(
        ...
        END,
        'is_system_user', CASE
          WHEN c.requester_id = p_user_id THEN COALESCE(helper.is_system_user, false)
          ELSE COALESCE(requester.is_system_user, false)
        END
      ),
      -- Help request info (null for system conversations)
      'help_request', CASE
  ...
  -- Join help request (will be null for system conversations)
...
-- Update comment
COMMENT ON FUNCTION list_conversations_v2(uuid, text) IS
  'Lists all conversations for a user including system conversations (welcome messages). Supports optional status filter.';
