...
export function PendingOfferCard({
  ...
  return (
    <article
      ...
      aria-label={`Offer from ${offer.helper_profile.name} for ${offer.help_request.title}`}
    >
      {/* Helper Info */}
      <div className="flex items-start gap-3">
        <div
          ...
          aria-hidden="true"
        >
          {offer.helper_profile.name.charAt(0).toUpperCase()}
        </div>
        <div className="flex-1 min-w-0">
          <h4 className="font-semibold text-secondary truncate">{offer.helper_profile.name}</h4>
          <div className="flex items-center gap-1 text-sm text-gray-600">
            <MapPin className="w-4 h-4 flex-shrink-0" aria-hidden="true" />
            <span className="truncate">{offer.helper_profile.location}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-2 pt-2">
        <button
          ...
          className="flex-1 bg-sage hover:bg-sage-dark text-white font-medium py-2 px-4 rounded-lg
                     ...
                     min-h-[44px]"
          aria-label={`Accept offer from ${offer.helper_profile.name}`}
        >
        ...
        <button
          ...
          className="flex-1 bg-gray-200 hover:bg-gray-300 text-secondary font-medium py-2 px-4 rounded-lg
                     ...
                     min-h-[44px]"
          aria-label={`Decline offer from ${offer.helper_profile.name}`}
        >
